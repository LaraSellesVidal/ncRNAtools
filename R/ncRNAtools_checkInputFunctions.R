...
checkInferenceEngine <- function(inferenceEngine, method, sequence) {
  if (method == "centroidFold" & !(inferenceEngine %in% c("CONTRAfold", "BL", "Turner"))) {
    stop("Please select a secondary structure inference engine compatible with CentroidFold. See documentation for details")
  }
  else if (method == "centroidFold" & !(inferenceEngine %in% c("CONTRAfold", "BL", "Turner"))) {
    stop("Please select a secondary structure inference engine compatible with CentroidHomfold. See documentation for details")
  }
  else if (method == "IPknot" & !(inferenceEngine %in% c("CONTRAfold", "BL", "Turner", "NUPACK"))) {
    stop("Please select a secondary structure inference engine compatible with IPknot. See documentation for details")
  }
...
checkAlignmentEngine <- function(alignmentEngine, method) {
  if (method != "centroidHomFold") {
    warning("Alignment engine ignored, since CentroidHomfold was not used for secondary structure prediction.")
  }
  else if (!alignmentEngine %in% c("CONTRAlign", "ProbCons")) {
    stop("Please select a valid engine for pairwise alignment during CentroidHomfold prediction. See documentation for details.")
  }
}
...
checkEValueRfamSearch <- function(eValueRfamSearch, method) {
  if (method != "centroidHomFold") {
    warning("E-value for searching the Rfam database ignored, since CentroidHomfold was not used for secondary structure prediction.")
  }
...
checkNumHomSeqsRfamSearch <- function(numHomSeqsRfamSearch, method) {
  if (method != "centroidHomFold") {
    warning("Number of homolog sequences for searching the Rfam database ignored, since CentroidHomfold was not used for secondary structure prediction.")
  }
...
checkBasicDotBracketString <- function(inputString) {
  if(!grepl('^[\\(\\)\\.]+$', gsub("[\r\n]", "", inputString))) {
    stop("Please provide a secondary structure string in the basic Dot-Bracket notation.")
  }
}
