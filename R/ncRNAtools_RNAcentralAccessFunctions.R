...
rnaCentralGenomicCoordinatesSearch <- function(genomicRanges, species) {
  if (class(genomicRanges) != "GRanges") {
  ...
  if (any(grepl("[^0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ]", chromosomes) & chromosomes != "MT")) {
    stop("Please enter valid chromosome names as seqnames for the GRanges object. See documentation for details.")
  }
  ...
  for (i in seq_len(length(genomicRanges))) {
    ...
    parsedResult <- parsedResult[grepl("^URS", sapply(parsedResult, `[`, "external_name"))]
    annotatedRNA[[i]] <- lapply(parsedResult, function(hit) list(rnaCentralID=strsplit(hit$ID, split="@")[[1]][1],
                                                                 species=species,
                                                                 description=hit$description,
                                                                 RNAType=hit$biotype,
                                                                 genomicCoordinates=GRanges(seqnames=Rle(chromosomes[i]),
                                                                                            ranges=IRanges(hit$start, hit$end),
                                                                                            strand=strsplit(hit$ID, split=":")[[1]][2])))
  }
